...
export default function Home() {
  ...
  return (
    <div className="relative h-screen w-full bg-[#EDEDED] overflow-hidden flex font-sans selection:bg-white selection:text-black">
      
      {/* MAIN SITE CANVAS FRAME - SHRINKS INTO ROUNDED CARD WHEN MENU IS OPEN */}
      <div
        onClick={() => menuOpen && setMenuOpen(false)}
        className={`relative flex-1 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] bg-[#0a0f0a] text-white flex flex-col justify-between overflow-hidden ${
          menuOpen
            ? "h-[calc(100vh-28px)] my-3.5 ml-3.5 rounded-[20px] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.7)] border border-white/20 cursor-pointer pointer-events-auto select-none"
            : "h-full my-0 ml-0 rounded-none shadow-none border-transparent"
        }`}
      >
        ...
        <div className="absolute inset-0 z-0">
          <Image
            ...
            fill
            priority
            className="object-cover object-center scale-100 filter brightness-[0.75] contrast-[1.1]"
          />
      ...
      {/* RIGHT SIDE MENU PANEL - ALWAYS MOUNTED WITH SMOOTH CLIPPED TRANSITION */}
      <div
        className={`h-full bg-[#EDEDED] text-black shrink-0 z-50 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] overflow-hidden ${
          menuOpen
            ? "w-[300px] sm:w-[360px] md:w-[400px] opacity-100 pointer-events-auto"
            : "w-0 opacity-0 pointer-events-none"
        }`}
      >
        {/* Inner panel with fixed width prevents content wrapping/reflow during width animation */}
